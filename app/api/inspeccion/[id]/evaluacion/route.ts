...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data: inspeccion, error: errInspeccion } = await supabase
    ...
    .select('id')
    .single();
  if (errInspeccion) return NextResponse.json({ error: errInspeccion.message }, { status: 500 });
  ...
  if (errElementos) return NextResponse.json({ error: errElementos.message }, { status: 500 });
